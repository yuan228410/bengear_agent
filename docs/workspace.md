...
1. 创建目录：`memory_data/`、`memory_data/sessions/`、`skills/`、`roles/`、`.team/inbox/`
2. 写入 `workspace.json`
3. 写入默认模板：
   - `SOUL.md` — "You are BenGear, a concise cross-platform coding agent."
   - `RULES.md` — 空
   - `MEMORY.md` — 空
   - `roles/lead.json` — `{"name": "lead", "tool_whitelist": []}`
   - `roles/teammate.json` — `{"name": "teammate", "tool_whitelist": ["read_file", "list_dir", "run_command", "http_get", "get_skill"]}`
...
```
<workspace>/memory_data/sessions/<session_id>/
├── meta.json                     # 会话元数据
└── memory_data/
    ├── 20260604.md               # 每日情景
    └── 20260605.md
...
3. **角色配合**：使用 `--role teammate` 在工作空间中创建受限 Agent
