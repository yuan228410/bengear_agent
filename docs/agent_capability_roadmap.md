...
- `Runtime` 基于已注册的 `CheckpointService` 为可识别路径的修改类工具自动创建 checkpoint，包括文件写删改、patch apply/revert、checkpoint restore、git restore 等。
...
- 提供 `diagnostic_repair_workflow` 自动修复闭环：生成修复计划、校验候选 patch、创建候选 checkpoint、通过治理链路应用安全 patch、重跑推荐测试；测试失败时可自动走治理链路恢复 checkpoint，并输出结构化总结与最终工作区状态。
- 限制最大重试次数。
- 最终报告测试结果和未解决问题。
