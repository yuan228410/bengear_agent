...
bool HttpParser::parse_method_line() {
    ...
    std::string line(buffer_.data(), line_end);
...
bool HttpParser::parse_body() {
    ...
    if (content_length_ > 0) {
        ...
        request_.body = std::string(buffer_.data(), content_length_);
...
bool HttpParser::parse_chunked_body() {
    while (true) {
        if (current_chunk_size_ == 0) {
            ...
            std::string size_line(buffer_.data(), line_end);
        ...
        request_.body.append(std::string(buffer_.data(), current_chunk_size_));
