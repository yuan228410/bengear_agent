...
TEST(GitApiTest, StatusParsesWorkspaceAndUsername) {
    ...
    svc.status = [](const std::string& workspace,
...
TEST(GitApiTest, DiffParsesWorkspacePathAndFlags) {
    server::Router router;
    server::GitApiService svc;
    svc.diff = [](const std::string& workspace,
                  const std::string& username,
    ...
    req.username = std::string("alice");
    req.query[std::string("workspace")] = std::string("default");
    req.query[std::string("path")] = std::string("src/main.cpp");
    req.query[std::string("staged")] = std::string("1");
    req.query[std::string("stat")] = std::string("0");
    req.query[std::string("preview")] = std::string("1");
...
TEST(GitApiTest, LogParsesWorkspacePathAndLimit) {
    server::Router router;
    server::GitApiService svc;
    svc.log = [](const std::string& workspace,
                 const std::string& username,
                 std::string_view path,
                 int limit) {
        EXPECT_EQ(workspace, std::string("default"));
...
TEST(GitApiTest, LogDefaultsInvalidLimit) {
    ...
    server::HttpRequest req;
    req.username = std::string("alice");
    req.query[std::string("limit")] = std::string("bad");
    auto* handler = router.match("GET", "/api/git/log", req);
    ASSERT_NE(handler, nullptr);
    auto resp = (*handler)(req);
    EXPECT_EQ(resp.status, 200);
...
TEST(GitApiTest, BranchesParsesWorkspaceAndUsername) {
    server::Router router;
    server::GitApiService svc;
    svc.branches = [](const std::string& workspace,
                      const std::string& username) {
        EXPECT_EQ(workspace, std::string("default"));
        EXPECT_EQ(username, std::string("alice"));
...
TEST(GitApiTest, WorktreesParsesWorkspaceAndUsername) {
    server::Router router;
    server::GitApiService svc;
    svc.worktrees = [](const std::string& workspace,
                       const std::string& username) {
        EXPECT_EQ(workspace, std::string("default"));
        EXPECT_EQ(username, std::string("alice"));
...
TEST(GitApiTest, CreateBranchParsesBodyAndCallsService) {
    server::Router router;
    server::GitApiService svc;
    svc.create_branch = [](const std::string& workspace,
...
TEST(GitApiTest, CreateBranchDelegatesGovernanceToService) {
    ...
    svc.create_branch = [&create_called](const std::string&, const std::string&, const std::string&, std::string_view, std::string_view, bool) {
        create_called = true;
...
TEST(GitApiTest, SwitchBranchParsesBodyAndCallsService) {
    server::Router router;
    server::GitApiService svc;
    svc.switch_branch = [](const std::string& workspace,
                           const std::string& session_id,
                           const std::string& username,
                           std::string_view name,
                           bool force) {
...
TEST(GitApiTest, SwitchBranchMissingSessionReturns400) {
    ...
    svc.switch_branch = [&switch_called](const std::string&, const std::string&, const std::string&, std::string_view, bool) {
        switch_called = true;
...
TEST(GitApiTest, DeleteBranchParsesBodyAndCallsService) {
    server::Router router;
    server::GitApiService svc;
    svc.delete_branch = [](const std::string& workspace,
                           const std::string& session_id,
                           const std::string& username,
                           std::string_view name,
                           bool force) {
...
TEST(GitApiTest, DeleteBranchDelegatesGovernanceToService) {
    ...
    svc.delete_branch = [&delete_called](const std::string&, const std::string&, const std::string&, std::string_view, bool) {
        delete_called = true;
...
TEST(GitApiTest, BranchMutationRoutesDelegateGovernanceToService) {
    ...
    create_req.username = std::string("alice");
    ...
    switch_req.username = std::string("alice");
...
TEST(GitApiTest, RestoreParsesBodyAndCallsService) {
    server::Router router;
    server::GitApiService svc;
    svc.restore = [](const std::string& workspace,
                     const std::string& session_id,
                     const std::string& username,
...
TEST(GitApiTest, RestoreDelegatesGovernanceToService) {
    ...
    svc.restore = [&restore_called](const std::string&, const std::string&, const std::string&, const std::vector<std::string>&, bool, bool) {
        restore_called = true;
...
TEST(GitApiTest, CommitParsesBodyAndCallsService) {
    server::Router router;
    server::GitApiService svc;
    svc.commit = [](const std::string& workspace,
                    const std::string& session_id,
                    const std::string& username,
...
TEST(GitApiTest, CommitDelegatesGovernanceToService) {
    ...
    svc.commit = [&commit_called](const std::string&, const std::string&, const std::string&, std::string_view, const std::vector<std::string>&, bool, bool) {
        commit_called = true;
...
TEST(PermissionApiTest, ListParsesWorkspaceSessionAndUsername) {
    ...
    svc.list_pending = [](const std::string& workspace,
                          const std::string& session_id,
                          const std::string& username) {
        EXPECT_EQ(workspace, std::string("default"));
        EXPECT_EQ(session_id, std::string("sid-1"));
    ...
    req.username = std::string("alice");
    req.query[std::string("workspace")] = std::string("default");
    req.query[std::string("session_id")] = std::string("sid-1");
...
TEST(PermissionApiTest, ListMissingSessionReturns400) {
    server::Router router;
    server::PermissionApiService svc;
    svc.list_pending = [](const std::string&, const std::string&, const std::string&) {
...
TEST(PermissionApiTest, ApproveParsesBodyAndPathPermissionId) {
    server::Router router;
    server::PermissionApiService svc;
    svc.approve = [](const std::string& workspace,
                     const std::string& session_id,
                     const std::string& username,
...
TEST(CheckpointApiTest, ListParsesWorkspaceSessionAndUsername) {
    ...
    svc.list = [](const std::string& workspace,
                  const std::string& session_id,
                  const std::string& username) {
        EXPECT_EQ(workspace, std::string("default"));
        EXPECT_EQ(session_id, std::string("sid-1"));
...
TEST(CheckpointApiTest, ReadParsesIdAndStripsContent) {
    server::Router router;
    server::CheckpointApiService svc;
    svc.read = [](const std::string& workspace,
                  const std::string& session_id,
                  const std::string& username,
...
TEST(CheckpointApiTest, RestoreDelegatesGovernanceToService) {
    ...
    server::CheckpointApiService svc;
    bool restore_called = false;
    svc.restore = [&restore_called](const std::string&, const std::string&, const std::string&, std::string_view, const std::vector<std::string>&, bool) {
        restore_called = true;
...
TEST(CheckpointApiTest, DeleteParsesBodyAndCallsService) {
    server::Router router;
    server::CheckpointApiService svc;
    svc.remove = [](const std::string& workspace,
                    const std::string& session_id,
                    const std::string& username,
                    std::string_view checkpoint_id) {
        EXPECT_EQ(workspace, std::string("default"));
...
TEST(CheckpointApiTest, DeleteDelegatesGovernanceToService) {
    ...
    server::CheckpointApiService svc;
    bool delete_called = false;
    svc.remove = [&delete_called](const std::string&, const std::string&, const std::string&, std::string_view) {
        delete_called = true;
...
TEST(TestLoopApiTest, InspectParsesWorkspaceAndUsername) {
    ...
    svc.inspect = [](const std::string& workspace,
                     const std::string& username) {
        EXPECT_EQ(workspace, std::string("default"));
        EXPECT_EQ(username, std::string("alice"));
...
TEST(TestLoopApiTest, RunParsesBodyAndCallsService) {
    server::Router router;
    server::TestLoopApiService svc;
    svc.run = [](const std::string& workspace,
                 const std::string& session_id,
                 const std::string& username,
...
TEST(TestLoopApiTest, RunDelegatesGovernanceToService) {
    ...
    svc.run = [&run_called](const std::string&, const std::string&, const std::string&, std::string_view, std::string_view, int, int) {
        run_called = true;
...
TEST(DiagnosticContextApiTest, RepairContextParsesWorkspaceAndBody) {
    ...
    svc.repair_context = [](const std::string& workspace,
                            const std::string& username,
                            const ben_gear::Json& request) {
        EXPECT_EQ(workspace, std::string("default"));
        EXPECT_EQ(username, std::string("alice"));
...
TEST(DiagnosticContextApiTest, RepairContextRejectsInvalidJson) {
    ...
    server::DiagnosticContextApiService svc;
    bool called = false;
    svc.repair_context = [&called](const std::string&, const std::string&, const ben_gear::Json&) {
        called = true;
        return ben_gear::Json{{"success", true}};
    };
    server::register_diagnostic_context_routes(router, svc);
...
TEST(DiagnosticContextApiTest, RepairContextDoesNotRequireRunTestsPermission) {
    ...
    server::DiagnosticContextApiService svc;
    int calls = 0;
    svc.repair_context = [&calls](const std::string&, const std::string&, const ben_gear::Json&) {
        ++calls;
...
TEST(DiagnosticRepairApiTest, RepairPlanParsesWorkspaceAndBody) {
    ...
    svc.repair_plan = [](const std::string& workspace,
                         const std::string& username,
                         const ben_gear::Json& request) {
        EXPECT_EQ(workspace, std::string("default"));
        EXPECT_EQ(username, std::string("alice"));
...
TEST(DiagnosticRepairApiTest, RepairPlanRejectsInvalidJson) {
    ...
    server::DiagnosticRepairApiService svc;
    bool called = false;
    svc.repair_plan = [&called](const std::string&, const std::string&, const ben_gear::Json&) {
        called = true;
        return ben_gear::Json{{"success", true}};
    };
    server::register_diagnostic_repair_routes(router, svc);
...
TEST(DiagnosticRepairApiTest, RepairPlanDoesNotRequireRunTestsPermission) {
    ...
    server::DiagnosticRepairApiService svc;
    int calls = 0;
    svc.repair_plan = [&calls](const std::string&, const std::string&, const ben_gear::Json&) {
        ++calls;
...
TEST(DiagnosticRepairApiTest, PatchPreviewParsesWorkspaceAndBody) {
    server::Router router;
    server::DiagnosticRepairApiService svc;
    svc.repair_patch_preview = [](const std::string& workspace,
                                  const std::string& username,
                                  const ben_gear::Json& request) {
        EXPECT_EQ(workspace, std::string("default"));
        EXPECT_EQ(username, std::string("alice"));
...
TEST(DiagnosticRepairApiTest, PatchPreviewMissingUnifiedDiffReturns400) {
    server::Router router;
    server::DiagnosticRepairApiService svc;
    svc.repair_patch_preview = [](const std::string&, const std::string&, const ben_gear::Json&) {
...
TEST(DiagnosticRepairApiTest, PatchPreviewDoesNotRequireRunTestsPermission) {
    ...
    server::DiagnosticRepairApiService svc;
    int calls = 0;
    svc.repair_patch_preview = [&calls](const std::string&, const std::string&, const ben_gear::Json&) {
        ++calls;
        return ben_gear::Json{{"success", true}, {"provider", "diagnostic_repair_patch_preview"}};
    };
    server::register_diagnostic_repair_routes(router, svc);
...
TEST(DiagnosticRepairApiTest, RepairWorkflowParsesWorkspaceAndBody) {
    server::Router router;
    server::DiagnosticRepairApiService svc;
    svc.repair_workflow = [](const std::string& workspace,
                             const std::string& username,
                             const ben_gear::Json& request) {
        EXPECT_EQ(workspace, std::string("default"));
        EXPECT_EQ(username, std::string("alice"));
...
TEST(AuditApiTest, EventsParsesFiltersAndUsername) {
    ...
    svc.list_events = [](const std::string& workspace,
                         const std::string& session_id,
                         const std::string& username,
        ...
        EXPECT_EQ(session_id, std::string("sid-1"));
        EXPECT_EQ(username, std::string("alice"));
        EXPECT_EQ(category, std::string("permission"));
        EXPECT_EQ(action, std::string("requested"));
        EXPECT_EQ(limit, 25);
    ...
    req.query[std::string("workspace")] = std::string("default");
    req.query[std::string("session_id")] = std::string("sid-1");
    req.query[std::string("category")] = std::string("permission");
    req.query[std::string("action")] = std::string("requested");
    req.query[std::string("limit")] = std::string("25");
...
TEST(AuditApiTest, EventsDefaultsLimit) {
    server::Router router;
    server::AuditApiService svc;
    svc.list_events = [](const std::string&,
                         const std::string&,
                         const std::string&,
                         const std::string&,
                         const std::string&,
...
TEST(PatchApiTest, ListChangesRequiresSessionId) {
    ...
    svc.list_changes = [](const std::string&, const std::string&, const std::string&) {
...
TEST(PatchApiTest, ReadChangeMapsNotFoundTo404) {
    server::Router router;
    server::PatchApiService svc;
    svc.read_change = [](const std::string& workspace,
                         const std::string& session_id,
                         const std::string& username,
...
TEST(PatchApiTest, PreviewApplyAndRevertParseBody) {
    server::Router router;
    server::PatchApiService svc;
    svc.preview_patch = [](const std::string& workspace,
                           const std::string& session_id,
                           const std::string& username,
    ...
    svc.apply_patch = [](const std::string& workspace,
                         const std::string& session_id,
                         const std::string& username,
                         std::string_view unified_diff,
    ...
    svc.revert_change = [](const std::string& workspace,
                           const std::string& session_id,
                           const std::string& username,
                           std::string_view change_id,
                           bool force) {
    ...
    preview_req.username = std::string("alice");
    ...
    apply_req.username = std::string("alice");
    ...
    revert_req.username = std::string("alice");
...
TEST(PatchApiTest, MutationRoutesDelegateGovernanceToApplicationService) {
    ...
    svc.apply_patch = [&apply_called](const std::string&, const std::string&, const std::string&, std::string_view, std::string_view) {
        apply_called = true;
        return ben_gear::Json{{"success", true}, {"change_id", "chg_1"}};
    };
    svc.revert_change = [&revert_called](const std::string&, const std::string&, const std::string&, std::string_view, bool) {
        revert_called = true;
        return ben_gear::Json{{"success", true}, {"change_id", "chg_1"}};
    };
    server::register_patch_routes(router, svc);
