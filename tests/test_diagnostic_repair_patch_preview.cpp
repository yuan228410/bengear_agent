...
ben_gear::workspace::WorkspaceContext make_ctx(const std::filesystem::path& root) {
    ...
    ctx.session_id = ben_gear::base::container::String("diagnostic-repair-patch-preview-test-session");
...
TEST_F(DiagnosticRepairPatchPreviewServiceTest, NotesCandidateFileMismatch) {
    ...
    EXPECT_THAT(result["notes"].dump().to_std_string(), testing::HasSubstr("does not touch"));
}
