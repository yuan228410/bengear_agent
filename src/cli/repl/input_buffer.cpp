#include "cli/repl/input_buffer.hpp"
...
bool is_leading(unsigned char c) { return c >= 0xC0; }
...
std::string_view InputBuffer::content() const { return {buf_.data(), buf_.size()}; }
size_t InputBuffer::cursor() const { return pos_; }
bool InputBuffer::empty() const { return buf_.empty(); }
size_t InputBuffer::size() const { return buf_.size(); }

/// 当前内容的显示宽度（列数），考虑 CJK 宽字符（缓存优化）
size_t InputBuffer::display_width() const {
...
/// 光标位置对应的显示列数（缓存优化）
size_t InputBuffer::cursor_col() const {
...
void InputBuffer::insert(std::string_view str) {
...
void InputBuffer::kill_to_end() {
    buf_.erase(pos_);
    invalidate_cache();
}

void InputBuffer::kill_to_start() {
    buf_.erase(0, pos_);
    pos_ = 0;
    invalidate_cache();
}

void InputBuffer::backspace_word() {
    // 逐字符往前跳过空格，记录删除起点
...
void InputBuffer::cursor_home()  { 
    pos_ = 0; 
    cursor_col_cache_valid_ = false;
}

void InputBuffer::cursor_end()   { 
    pos_ = buf_.size(); 
    cursor_col_cache_valid_ = false;
}
...
void InputBuffer::set(std::string_view str) {
    buf_.clear();
...
size_t InputBuffer::prev_char_pos(size_t pos) const {
    if (pos == 0) return 0;
    --pos;
...
size_t InputBuffer::next_char_pos(size_t pos) const {
...
/// 失效所有缓存
void InputBuffer::invalidate_cache() {
