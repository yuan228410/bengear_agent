...
void RuntimePresenter::on_event(const base::core::RuntimeEvent& event) const {
    // CLI 只做结构化运行事件的轻量呈现；核心数据不包含任何终端/ANSI/UI 细节。
    if (event.kind == base::core::RuntimeEventKind::state_changed) return;
    stream_ << "[runtime] " << base::core::to_string(event.kind)
            << " status=" << base::core::to_string(event.status);
