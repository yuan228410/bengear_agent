        ...
        if (resp.status == 200 && !resp.body.empty()) {
            if (expect_zip && resp.body.size() >= 4) {
                ...
                if (p[0] != 0x50 || p[1] != 0x4B || p[2] != 0x03 || p[3] != 0x04) {
                    log::warn_fmt(
                        "HttpClient downloaded non-zip content ({} bytes, "
                        "magic={:02x}{:02x}{:02x}{:02x}), likely CDN error response",
            ...
            if (of) {
                of.write(resp.body.data(),
                         static_cast<std::streamsize>(resp.body.size()));
                log::info_fmt("downloaded via HttpClient: {} ({} bytes)",
                              url, resp.body.size());
                return true;
            }
        }
...
try_curl:
    // 2. 回退到 curl
    {
        ...
        if (std::system(curl_cmd.c_str()) == 0 &&
            ...
            std::filesystem::file_size(dest) > 0) {
    ...
    {
        std::string wget_cmd =
            "wget -q -O \"" + dest.string() + "\" \"" + url + "\""
#if BEN_GEAR_PLATFORM_POSIX
            " 2>/dev/null"
#endif
            ;
        if (std::system(wget_cmd.c_str()) == 0 &&
            std::filesystem::exists(dest) &&
            std::filesystem::file_size(dest) > 0) {
...
bool extract_zip(const std::filesystem::path& zip_path,
    ...
    // 校验 zip magic number
    {
        ...
        if (n < 4 || m0 != 0x50 || m1 != 0x4B || m2 != 0x03 ||
            m3 != 0x04) {
            log::error_fmt(
                "not a valid zip file: {} (magic={:02x}{:02x}{:02x}{:02x}), "
                "downloaded content may be an error response",
    ...
    // 预构建 central directory 映射
    struct CdEntry {
        uint32_t comp_size;
        uint32_t uncomp_size;
    };
    std::map<std::string, CdEntry> cd_map;

    // 从文件末尾搜索 End of Central Directory
    size_t eocd_pos = data.size() < 22 ? data.size() : data.size() - 22;
    for (; eocd_pos > 0; --eocd_pos) {
        if (data.size() >= eocd_pos + 4 &&
            read_u32_le(&data[eocd_pos]) == kEndCentralDirSig) {
            break;
        }
    }
    if (eocd_pos < data.size()) {
        ...
        for (uint16_t i = 0;
             i < cd_entries && cd_pos + 46 <= data.size(); ++i) {
            ...
            if (cd_pos + 46 + cd_name_len <= data.size()) {
                std::string cd_name(
                    reinterpret_cast<const char*>(&data[cd_pos + 46]),
                    cd_name_len);
    ...
    while (pos + 30 <= data.size()) {
        ...
        if (sig != kLocalFileHeaderSig) {
            log::debug_fmt("unexpected signature at offset {}: 0x{:08x}",
                           pos, sig);
            break;
        }
        ...
        // data descriptor: 从 central directory 获取真实大小
        if (flags & 0x08) {
            size_t name_start_tmp = pos + 30;
            if (name_start_tmp + name_len <= data.size()) {
                std::string tmp_name(
                    reinterpret_cast<const char*>(&data[name_start_tmp]),
                    name_len);
                ...
                if (it != cd_map.end()) {
                    ...
                    log::warn_fmt(
                        "data descriptor entry '{}' not found in central "
                        "directory, skipping",
                        tmp_name);
                    break;
                }
        ...
        std::string entry_name(
            reinterpret_cast<const char*>(&data[name_start]), name_len);
        ...
        // zip slip 安全检查
        auto out_path =
            std::filesystem::weakly_canonical(target_dir / entry_name);
        auto canonical_target =
            std::filesystem::weakly_canonical(target_dir);
        if (out_path.string().find(canonical_target.string()) != 0) {
            log::error_fmt("zip slip detected: {} escapes target dir",
                           entry_name);
        ...
        if (is_dir) {
            ...
            if (method == kMethodStored) {
                ...
                log::error_fmt(
                    "unsupported compression method {} for: {}", method,
                    entry_name);
            }
        }
        ...
        entry_count++;
        pos = data_start + comp_size;
        // data descriptor: 跳过 trailing data descriptor
        if (flags & 0x08) {
            if (pos + 24 <= data.size()) {
    ...
    log::info_fmt("zip extraction complete: {} entries from {}",
                  entry_count, zip_path.string());
