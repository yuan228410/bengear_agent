...
- ✅ **终端富文本** - Markdown 实时渲染（标题/表格/列表/代码高亮）、thinking 可折叠、Dracula 主题
...
- [CLI 参考](docs/cli.md) - 所有 CLI 选项和子命令
...
- [基础组件](docs/base_components.md) - 容器、内存池、日志等
...
├── mcp/            # MCP 协议客户端
├── config/         # 配置加载
├── cli/            # CLI 解析器 + 终端渲染器（Markdown/主题/语法高亮）
└── base/           # 基础组件（网络、日志、容器等）
```
